...
const Home = () => {
    ...
    const addToNodesRef = (el) => {
        if (el && !processNodes.current.includes(el)) {
            ...
            gsap.set(el, { scale: 0.8, rotation: 5, boxShadow: '0 0 0px transparent' });
        }
    };
    ...
    useEffect(() => {
        ...
        gsap.fromTo(ctaRef.current, { opacity: 0, scale: 0.9 }, {
            opacity: 1,
            scale: 1,
            duration: 1,
            ease: 'power3.out',
        ...
        <div className="zara-portal">
            ...
            <section className="zara-hero" ref={heroRef}>
                ...
                {[...Array(6)].map((_, i) => (
                    <div
                        ...
                        ref={addToFloatingObjectsRef}
                        style={{ '--color': i % 2 === 0 ? '#8ec5fc' : '#e0c3fc' }}
                    ></div>
                ))}
            </section>
            ...
            <section className="services-section" id="servicesAbout">
                ...
                <div className="services-grid" ref={servicesRef}>
                    <ServiceCard icon={<FaUserTie />} title="Internship" description="Permission for internship applications" color="#547792" />
                    <ServiceCard icon={<FaIdCard />} title="ID Card" description="Request new or replacement IDs" color="#94B4C1" />
                    <ServiceCard icon={<FaCode />} title="Hackathon" description="Register for hackathon & competitions" color="#547792" />
                    <ServiceCard icon={<FaCalendarAlt />} title="Leave" description="Apply for academic leave" color="#94B4C1" />
                </div>
            </section>
            ...
            <section className="process-section">
                ...
                <div className="process-map-container">
                    <div className="process-map" ref={processRef}>
                        {[...Array(5)].map((_, index) => (
                            <>
                                <div className={`process-node ${index === 0 ? 'first' : ''} ${index === 4 ? 'last' : ''}`} ref={addToNodesRef}>
                                    <div className="node-circle">{index + 1}</div>
                                    <div className="node-content">
                                        <h4>{['Authenticate', 'Verify Email', 'Select Service', 'Complete Form', 'Submit & Track'][index]}</h4>
                                        <p>{[
                                            'Register/Login With Your Credentials',
                                            'Enter OTP Sent To Your Mail',
                                            'Choose From Our Service Offerings',
                                            'Fill In The Required Details',
                                            'Send Request & Monitor Progress'
                                        ][index]}</p>
                                    </div>
                                </div>
                                {index < 4 && (
